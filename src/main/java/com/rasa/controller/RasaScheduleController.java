...
import com.rasa.constants.MessageCodes;
import com.rasa.models.CommonExample;
import com.rasa.models.Message;
import com.rasa.models.TrainRequest;
import com.rasa.models.existing.ExistingModelRoot;
import com.rasa.service.RasaSchedulerService;
import com.rasa.utils.JsonUtility;

@RestController
public class RasaScheduleController {

	private static final Logger LOGGER = LoggerFactory.getLogger(RasaScheduleController.class);
	...
	// @Autowired
	// @Qualifier("rasaSchedulerService")
	private RasaSchedulerService rasaSchedulerService = new RasaSchedulerService();;
	...
	public ResponseEntity<Message> trainNlu(@RequestBody List<CommonExample> commonExamplesList) {
		...
		TrainRequest trainRequest = rasaSchedulerService.getTrainRequestInstance(commonExamplesList, NLU_LANGUAGE,
				ML_PIPELINE);
		LOGGER.info("parsed new trained data  " + JsonUtility.toJson(trainRequest));

		// step 1 Reading existing model
		Message message = rasaSchedulerService.readExistingModelFromNlu();
		if (!message.isFlag()) {
		...
		try {
			String messageFromNlu = rasaSchedulerService.trainNLU(mergedTrainingDataInJson, NLU_URL, PROJECT_NAME,
					MODEL_NAME);
		...
		// step 5 push to git
		Message gitMessage = rasaSchedulerService.commitLatestTrainedDataToGitLab(mergedTrainingDataInJson,
